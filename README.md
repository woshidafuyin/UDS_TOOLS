...
当前代码分支：`main`
源码维护仓库：公司 GitLab `zhoufuyin/UDS_TOOLS`；本 README 描述保留全部厂商的通用主线。
...
- 从外部选择刷写资源后，工具先按原文件名原子复制到当前 Profile 的默认资源目录，再保存该字段的选择；外部源文件、Profile 默认文件、历史选择文件以及其他字段或项目的资源均不会被删除；当外部来源与已有目标同名时，保留原文件名并创建 `selected-XXXXXX` 独立子目录，避免覆盖其他字段、目标或保存任务仍引用的文件；直接选择同一个受管文件时复用原路径；
...
- 当前主线 CTest 共 8 项：核心、P416、CAN 适配、厂商 API 边界、厂商清单、应用状态、Qt 探测桥接和 Qt 主窗口；另有 `tests/resource_file_collision_tests.cpp` 的独立 MSVC/Qt 同名文件导入专项，本次同步单独编译运行；
...
## 16. 2026-09-05 公司主线同步范围

- 同步已完成的版本读取反馈整合与同名资源导入隔离，保留全部22个项目Profile、17个Workflow及四个功能页。
- 版本读取分别展示原始ECU响应和失败详情；未执行、取消、警告、失败不混作正常响应。
- 同名文件隔离保护默认资源、不同字段的历史选择和外部来源文件；此修复不等于补齐所有空默认路径的导入行为。
- 此条为当时交付范围记录；Perodua P02C 后续已接入主线，当前支持范围及待提供输入见 [P02C 说明](docs/PERODUA_P02C_FLASH.md)。选择项目后可通过“项目刷写参数…”保存 CRC、测试仪身份和所需 BIN 地址，立即生效。
- KP31单独交付分支、奇瑞厂商裁剪版及其启动最大化/100ms唤醒变更，未在本次同步中合并到通用主线。
- 本次为源码与文档同步，不重新发布或覆盖正在使用的dist；详情见 `docs/MAINLINE_GITLAB_SYNC_20260905.md`。

## 17. 通用刷写前置提示（2026-09-05）

所有已接入项目的“能否刷写”和“开始刷写”共用模式及文件前置校验。未选模式时点击按钮，右侧运行日志显示红色提示；当前模式缺少必需文件时一次列出所有缺项。已选路径被删除、是目录或不可读取时也显示红色错误。校验未通过不会发起本次在线探测或刷写请求。

文件需求仍按项目及 APP/CAL/APP+CAL 模式判断：保留零跑可选验签、TMP 内置验签和楚能 CBF 内置验签规则。KP31 不依赖默认资源路径非空来判断必填文件。提示同时保存到执行日志；版本读取不要求选择刷写文件。

## 2026-09-07 公盘基准与正式目录

E0Y与KP31分别按公盘同名目录核对。E0Y正常UDS骨架与八月包一致，另有用户确认实际刷写必需的唤醒与就绪探测；KP31主要沿用四月包，八月包存在E0Y衍生适配疑点，不能按日期直接替换流程。详见docs/CHERY_PUBLIC_SHARE_AUDIT_20260907.md。正式交付只使用dist；代码通过Git提交记录和回退，不保留多份旁路运行目录。
