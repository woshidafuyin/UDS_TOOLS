...
void Chuneng331Flow::enter_programming_session(
  ...
  if (entry.run_standard_preprogramming) {
    expect(functional_, kChuneng331FunctionalDefaultSessionRequest,
           std::array<std::uint8_t, 2>{0x50, 0x01}, 1,
           "10 01 Functional DefaultSession");
    // CANoe's standard APP entry sends the functional ExtendedSession with
    // suppressPositiveResponse set.  Do not wait on the selected physical RX
    // ID for this broadcast step; the on-wire request is 10 83.
    ...
    expect_routine(physical_, kChuneng331ProgrammingPrecondition,
                   0x0203, 4, "31 01 02 03 ProgrammingPrecondition");
    send_functional_no_response(
...
void Chuneng331Flow::run(const Chuneng331Images& images,
  ...
  const auto write_fingerprint = [this] {
    ...
    expect(physical_, f184, f184_prefix, 20,
           "2E F184 Fingerprint (9 bytes)");
  };

  // The current ARC331 sequence writes the programming fingerprint before
  // the first RequestDownload. Keep it independent of the input container:
  // CBF and S-record jobs enter this same state machine.
  write_fingerprint();
  check_wakeup();
